...
Client.prototype.getBuyPrice = function(params, callback) {
  ...
  // For backward compatibility, if params.currency does not contain '-', default to BTC-XXX format
  var currency = params.currency ? params.currency : 'BTC-USD';
  currency = currency.indexOf('-') === -1 ? 'BTC-' + currency : currency;
  this._getOneHttp({'path': 'prices/' + currency + '/buy'}, callback);
};
...
Client.prototype.getSellPrice = function(params, callback) {
  ...
  var currency = params.currency ? params.currency : 'BTC-USD';
  currency = currency.indexOf('-') === -1 ? 'BTC-' + currency : currency;
  this._getOneHttp({'path': 'prices/' + currency + '/sell'}, callback);
};

Client.prototype.getSpotPrice = function(params, callback) {

  var currency = params && params.currency ? params.currency : 'BTC-USD';
  currency = currency.indexOf('-') === -1 ? 'BTC-' + currency : currency;
  this._getOneHttp({'path': 'prices/' + currency + '/spot'}, callback);
};
