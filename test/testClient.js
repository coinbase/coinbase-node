...
describe('Client', function() {
  ...
  describe('client methods without authentication', function() {

    var client = new Client({'baseApiUri': na.TEST_BASE_URI});
    it('should not get accounts and give invalid token error', function() {
      client.getAccounts({}, function(err, accounts) {
        assert.equal(err.message, "The access token is invalid");
        assert.equal(err.status, 401);
      });
    });

    it('should not get account and give invalid token error', function() {
      client.getAccount(na.ACCOUNTS_ID_1, function(err, account) {
        assert.equal(err.message, "The access token is invalid");
        assert.equal(err.status, 401);
      });
    ...
    it('should not create account', function() {
      var args = {
        "name": na.NEW_ACCOUNT_NAME_1
      };
      client.createAccount(args, function(err, account) {
    ...
    it('should not get current user information', function() {
      client.getCurrentUser(function(err, user) {
        assert.equal(err.message, "The access token is invalid");
        assert.equal(err.status, 401);
      });
    ...
    it('should not get payment methods because the client is not authenticated', function() {
      client.getPaymentMethods({}, function(err, pms) {
        assert.equal(err.message, "The access token is invalid");
        assert.equal(err.status, 401);
      });
    ...
    it('should not get a payment method because the client is not authenticated', function() {
      client.getPaymentMethod(na.GET_PAYMENT_METHOD_RESP.data.id, function(err, pm) {
        assert.equal(err.message, "The access token is invalid");
        assert.equal(err.status, 401);
      });
    });
